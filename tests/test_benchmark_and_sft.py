...
def test_benchmark_runner_outputs_metrics_for_algorithms(tmp_path):
    ...
    assert {row["algorithm"] for row in rows} == {"first-fit", "balanced-fit", "AI-phase2", "AI-phase3"}
    assert all("fallback_rate" in row for row in rows)
    assert all("avg_latency_ms" in row for row in rows)
    assert all("agent_escalation_rate" in row for row in rows)
    assert all("memory_usage_rate" in row for row in rows)
    assert all("avg_retrieved_episode_count" in row for row in rows)
